...
ax.set_title('Soft/high state', size=18)
...
fmt_disk={'color':'darkred','linestyle':'solid', 'alpha':0.5}
...
#Ns = 5
#for i, rho in enumerate( np.linspace(0.05, 0.2, Ns) ):
#    fmtS['facecolor'] = cmap(i / float(Ns-1) )
#    for spot_phi in np.linspace(0.0, 2.0*pi, 20):
#        ax = nspy.draw_spot(ax, spot_phi, spot_theta, rho, fmt=fmtS)


savefig('fig_disk_soft.png', bbox_inches='tight')
